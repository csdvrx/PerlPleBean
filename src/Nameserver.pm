...
sub unbinip {
 ...
 if ($addr_family == 2) { # AF_INET means IPv4
        #decode remaining
        ...
        return ($ipstr);
# } elsif ($addr_family == 10) { # AF_INET6 means IPv6
#        #decode remaining
#        my @decoded_addr = unpack("xxnHHHHHHHH", $ip); 
#        #print("Remote port: $decoded_addr[0]\n");
#        my $ipstr = join(":", @decoded_addr);
#        #print("Remote IP: $ipstr\n");
#        return ($ipstr);
 } else { # AF_INET
...
# FIXME: according to the documentation, should accept either, but accepts neither in APE:
#my $bip=inet_ntoa($MultiCast); #  Bad arg length for Socket::inet_ntoa, length is 22, should be 4
#my $bip=inet_aton($MultiCast); #  Bad arg length for Socket::inet_ntoa, length is 22, should be 4 at /zip/lib/perl5/5.36.0/x86_64-cosmo/IO/Socket/Multicast.pm line 59.
#my $bip=unbinip($MultiCast); # same
#my $destination_cc = sockaddr_in($sock->sockport,$sock->peerhost);
#my $destination_packed = sockaddr_in($port,inet_aton($MultiCast));
#$sock_udp_mcast->mcast_add(\$destination_packed) or die "Couldn't add $$MultiCast: $!\n";
#$sock_udp_mcast->mcast_add(\$bip) or die "Couldn't add $$MultiCast: $!\n";
#$sock_udp_mcast->mcast_add('224.0.0.251') or die "Couldn't add $$MultiCast: $!\n";
...
if (defined($self->{MCastAddr})) {
  ...
  # FIXME: check how beneficial it is to also reply to the client - it seems to help windows at least
  # mac-like bonjour seems to be indifferent to the mismatched ip for the 1st, 2nd and 3rd reply (tested with 127.2.2.2)
  # currently testing spreadsheet.local which only does the 4th and 5th bonjour reply to see if the mismatching
  # emitter of the CC packet will cause issues: may need to replace the simple set of 3 sockets by a set for each
  # .local domain using a custom IP from 127/8
  my $destination_cc = sockaddr_in($sock->sockport,$sock->peerhost);
  print $sock->send( $reply->data($max_len), $destination_cc) ? "done cc udp" : "failed cc udp: $!", "\n";
