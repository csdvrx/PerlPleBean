...
# FIXME: need to be able to specify what to execute 
# but for cgi, couldn't it just be checking env?
...
my $pid="";
print STDOUT "Regular fork with no begin\n";
# If doing fork and exit, won't execute main
if ($fork_and_exit) {
 $pid = fork and exit;
} else {
 $pid = fork;
...
if ($pid == 0) {
 print STDOUT "Child\n";
 if ($run_in_child) {
  # FIXME: but do we want to run in main or child?
  if (scalar(@ARGV)>0) {
   my $fpart0=$0;
